...
func main() {
	...
	store := booking.NewStore(db, tracer)
	client := booking.NewClient(consulRegistry, tracer)
	service := booking.NewService(store, client, tracer)
	handler := booking.NewHandler(service, tracer)
